    ...
    <div className="min-h-screen flex items-center justify-center px-6 py-24">
      <div className="max-w-md w-full text-center space-y-6">
        <div className="space-y-2">
          <h1 className="text-4xl font-bold text-[color:var(--foreground)]">Algo deu errado</h1>
          <p className="text-[color:var(--muted)] text-lg">Ocorreu um erro inesperado</p>
        </div>
        ...
        {process.env.NODE_ENV === "development" && (
          <div className="p-4 bg-[color:var(--surface)] rounded-lg border border-[color:var(--border)] text-left shadow-sm">
            ...
            {error.digest && (
              <p className="text-xs text-[color:var(--muted)] mt-2">Digest: {error.digest}</p>
            )}
          </div>
        )}
